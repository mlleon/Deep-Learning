...
def create_model(num_classes):
    ...
    # 在rpn_function.py中set_cell_anchors函数中有一层for遍历，所以size和aspect_ratios都是((value),)的形式,多套一层括号。
    anchor_generator = AnchorsGenerator(sizes=((32, 64, 128, 256, 512),),
                                        aspect_ratios=((0.5, 1.0, 2.0),))

    roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'],  # 在哪些特征层上进行roi pooling(对应faster_rcnn_framework.py中88行的feature)
                                                    output_size=[7, 7],   # roi_pooling输出特征矩阵尺寸(输出特征矩阵的高和宽)
...
def main():
    ...
    if aspect_ratio_group_factor >= 0:
        ...
        # group_ids结果：[4, 5, 6, 2, 5, 5, 5, 4 ...] 统计所有图像高宽比在bins区间中的组索引
