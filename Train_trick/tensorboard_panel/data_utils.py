...
# 绘制部分data_loader图片，观察图片基本情况
def plot_data_loader_image(data_loader):
    batch_size = data_loader.batch_size
    plot_num = min(batch_size, 4)  # 最多绘制4张图片
...
#  绘制需要预测的图片，并获得预测对应的准确率
def plot_class_preds(net,  # 实例化的模型
                     images_dir: str,  # 预测图片的根目录
                     transform,  # 验证集使用的图像预处理
                     num_plot: int = 5,  # 展示几张图片
                     device="cpu"):
    ...
    """
        label_info = 
            [['/home/leon/Deep_Learning/classification/tensorboard_test/predict_img/1.jpeg', 'tulips'], 
            ['/home/leon/Deep_Learning/classification/tensorboard_test/predict_img/2.jpeg', 'roses']]
    """
    if len(label_info) == 0:
        return None
    ...
    for img_path, class_name in label_info:
        ...
        label_index = int(class_indices[class_name])    # 读取对应的图片索引
    ...
    # batching images(把所有测试图片组成一个batch)
