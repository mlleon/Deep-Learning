...
class SummaryWriter(object):
    ...
    def __init__(self, log_dir=None, comment='', purge_step=None, max_queue=10,
        ...
        参数：
            log_dir(string): -保存目录位置。默认是运行/CURRENT_DATETIME_HOSTNAME，每次运行后都会发生变化。
            使用分层文件夹结构轻松比较运行。例如为每个新实验传入“runs/exp1”、“runs/exp2”等，以便在它们之间进行比较。
            comment(string): -注释 log_dir 后缀附加到默认 log_dir 。如果指定了log_dir，则此参数无效。
            purge_step(int): -当记录在步骤T+X崩溃并在步骤 T 重新启动时，任何 global_step
            大于或等于 T 的事件都将从 TensorBoard 中清除并隐藏。请注意，崩溃和恢复的实验应该具有相同的 log_dir 。
            max_queue(int): -在 ‘add’ 调用之一强制刷新到磁盘之前，挂起事件和摘要的队列大小。默认为十项。
            flush_secs(int): -将挂起的事件和摘要刷新到磁盘的频率(以秒为单位)。默认为每两分钟一次。
            filename_suffix(string): -后缀添加到log_dir 目录中的所有事件文件名。
            有关 tensorboard.summary.writer.event_file_writer.EventFileWriter 中文件名构造的更多详细信息。
    ...
    def add_hparams(
        ...
        参数：
            hparam_dict(dict) -字典中的每个键值对都是超参数的名称及其对应的值。
                值的类型可以是 bool 、 string 、 float 、 int 或 None 之一。
            metric_dict(dict) -字典中的每个键值对都是度量的名称及其对应的值。
                注意这里使用的键在张量板记录中应该是唯一的。否则，您通过add_scalar 添加的值将显示在 hparam 插件中。在大多数情况下，这是不需要的。
            hparam_domain_discrete-(可选[Dict[str, List[Any]]])一个字典，包含超参数的名称和它们可以保存的所有离散值
    ...
    def add_scalar(self, tag, scalar_value, global_step=None, walltime=None):
        ...
        参数：
            tag(string) -数据标识符
            scalar_value(float或者字符串/blob 名称) -保存值
            global_step(int) -要记录的全局步长 值
            walltime(float) -可选的覆盖默认的墙时间(时间。time())在事件纪元之后的秒数
            new_style(布尔值) -是使用新样式(张量字段)还是旧样式(simple_value 字段)。新样式可能会导致更快的数据加载。
    ...
    def add_histogram(self, tag, values, global_step=None, bins='tensorflow', walltime=None, max_bins=None):
        ...
        参数：
            tag(string) -数据标识符
            values(torch.Tensor,numpy.array， 或者字符串/blob 名称) -建立直方图的值
            global_step(int) -要记录的全局步长 值
            bins(string) -{‘tensorflow’,'auto', ‘fd’, ...} 之一。这决定了如何制作箱子。
                您可以在以下位置找到其他选项：https://docs.scipy.org/doc/numpy/reference/generated/numpy.histogram.html
    ...
    def add_image(self, tag, img_tensor, global_step=None, walltime=None, dataformats='CHW'):
        ...
        参数：
            ...
            global_step(int) -要记录的全局步长 值
            walltime(float) -事件纪元后的可选覆盖默认 walltime (time.time()) 秒将图像数据添加到摘要中。

            请注意，这需要pillow 包。

        形状：
            img_tensor：默认为  。您可以使用 torchvision.utils.make_grid() 将一批张量转换为 3xHxW
            格式或调用 add_images 让我们完成这项工作。只要传递了相应的 dataformats 参数，
            带有（1, H, W）、(H, W) 、(H, W, 3) 的张量也适用，例如CHW，HWC，HW。
    ...
    def add_images(self, tag, img_tensor, global_step=None, walltime=None, dataformats='NCHW'):
        ...
        形状：
            img_tensor：默认为(N, 3, H, W) 。如果指定dataformats，则接受其他形状。例如NCHW 或 NHWC。
